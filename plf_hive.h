...
private:
	...
	void reset_group_numbers() noexcept
	{
		...
		{
			current_group->group_number = group_number++;
		}
	}
	...
	iterator insert(const element_type &element)
	{
		if (end_iterator.element_pointer != nullptr)
		{
			if (groups_with_erasures_list_head == nullptr) // ie. there are no erased elements and end_iterator is not at end of current final group
			{
				...
				{
					std::allocator_traits<allocator_type>::construct(*this, bitcast_pointer<pointer>(unused_groups_head->elements), element);
	...
	iterator insert(element_type &&element) // The move-insert function is near-identical to the regular insert function, with the exception of the element construction method and is_nothrow tests.
	{
		if (end_iterator.element_pointer != nullptr)
		{
			if (groups_with_erasures_list_head == nullptr)
			{
				...
				{
					std::allocator_traits<allocator_type>::construct(*this, bitcast_pointer<pointer>(unused_groups_head->elements), std::move(element));
					next_group = reuse_unused_group();
				}
	...
	iterator emplace(arguments &&... parameters) // The emplace function is near-identical to the regular insert function, with the exception of the element construction method, and change to is_nothrow tests.
	{
		if (end_iterator.element_pointer != nullptr)
		{
			if (groups_with_erasures_list_head == nullptr)
			{
				...
				{
					std::allocator_traits<allocator_type>::construct(*this, bitcast_pointer<pointer>(unused_groups_head->elements), std::forward<arguments>(parameters) ...);
					next_group = reuse_unused_group();
				}
...
private:
	...
	void update_subsequent_group_numbers(size_type current_group_number, group_pointer_type update_group) noexcept // used by splice()
	{
		do
		{
